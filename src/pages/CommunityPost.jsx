...
export default function CommunityPost() {
  ...
  const renderMedia = (post) => {
    // Agar media array khali hai toh purani image ko fallback banayenge
    ...
    return (
      <Swiper
        ...
        navigation={true}
        className="w-full h-auto max-h-[600px] bg-black group"
      >
        {mediaItems.map((item, idx) => (
          <SwiperSlide key={idx} className="flex items-center justify-center bg-gray-900">
            {item.type === 'video' ? (
              <video 
                src={item.url} 
              ...
              <div className="relative w-full pt-[125%] h-0 overflow-hidden bg-black">
                 <iframe 
              ...
              <img 
                src={item.url} 
                alt="post media"
                className="w-full h-full object-contain block transition-transform duration-700 hover:scale-[1.02]" 
              />
            )}
          </SwiperSlide>
        ))}
        ...
        <div className="flex flex-col gap-3 min-w-[220px] p-1">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col text-black">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none text-black">Status Change</span>
              <span className="text-[13px] font-black text-gray-800 leading-tight mt-1 text-black">
                Move to <span className="text-red-500 uppercase italic">{level}</span>?
              </span>
            </div>
          </div>
          <div className="flex gap-2 mt-1">
            <button onClick={() => { toast.dismiss(t.id); submitStatUpdate(postId, level); }} className="flex-1 bg-black text-white py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter shadow-lg">Update</button>
            <button onClick={() => toast.dismiss(t.id)} className="flex-1 bg-gray-100 text-gray-400 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter">Deny</button>
          </div>
        </div>
      ), { duration: 5000, position: 'top-center', style: { borderRadius: '24px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', border: '1px solid #f0f0f0' } });
      return;
    }
        ...
        {dbPosts.map((post) => {
          ...
          return (
            <div key={post._id} className="mb-12 border-b border-gray-100 pb-6">
              ...
              {/* --- 📸 MULTI-MEDIA SWIPER --- */}
              <div 
                className="relative w-full bg-gray-100 overflow-hidden shadow-inner" 
              ...
              <div className="px-5 py-4">
                ...
                <div className="flex flex-col gap-0">
                  <h2 className="w-fit text-5xl font-black text-gray-900 uppercase tracking-tighter leading-[0.85] mb-2 transform transition-transform hover:translate-x-1 italic underline decoration-blue-500 decoration-8 underline-offset-4">
                    {post.word}
                  </h2>
              ...
              <div className={`px-4 mt-2 overflow-hidden transition-all duration-500 ${isOpen ? "max-h-60" : "max-h-0"}`}>
                <div className="bg-gray-50/50 backdrop-blur-sm rounded-[2rem] p-4 grid grid-cols-2 gap-3 border border-gray-100">
                  <button onClick={(e) => handleStatUpdate(e, post._id, 'easy')} className={`flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all duration-300 ${userLevel === 'easy' ? 'border-green-500 bg-white shadow-md scale-[1.02]' : 'border-transparent bg-white/50 hover:bg-white'}`}>
                  ...
                  <button onClick={(e) => handleStatUpdate(e, post._id, 'hard')} className={`flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all duration-300 ${userLevel === 'hard' ? 'border-red-500 bg-white shadow-md scale-[1.02]' : 'border-transparent bg-white/50 hover:bg-white'}`}>
                  ...
                  <button onClick={(e) => handleStatUpdate(e, post._id, 'heard')} className={`flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all duration-300 ${userLevel === 'heard' ? 'border-orange-400 bg-white shadow-md scale-[1.02]' : 'border-transparent bg-white/50 hover:bg-white'}`}>
                  ...
                  <button onClick={(e) => handleStatUpdate(e, post._id, 'dailyUse')} className={`flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all duration-300 ${userLevel === 'dailyUse' ? 'border-blue-500 bg-white shadow-md scale-[1.02]' : 'border-transparent bg-white/50 hover:bg-white'}`}>
