...
import { 
  ...
  Mic,          // 🔥 New Icon
  MicOff,       // 🔥 New Icon
  Volume1       // 🔥 New Icon for Pronunciation
} from 'lucide-react';
...
export default function LessonsPage() {
  ...
  const [activeSpeakingLesson, setActiveSpeakingLesson] = useState(null); // 🔥 New state for Voice Practice
  ...
  // 🔥 UPDATED VOICE ENGINE FUNCTION - Speaks English Now
  const speakEnglishSentence = (text) => {
    if (!('speechSynthesis' in window)) return;
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = "en-US"; // English language for correct pronunciation
    utterance.rate = 0.85;
    window.speechSynthesis.speak(utterance);
  };

  const checkPronunciation = (spoken, target) => {
    // Normalizing text: removing punctuation, making lowercase for accurate matching
    ...
    if (normalize(spoken) === normalize(target)) {
      ...
      toast.error("Not quite right. Try speaking slowly.");
    }
  };

  const startListening = (targetSentence) => {
    ...
    recognition.onresult = (event) => {
      ...
      checkPronunciation(transcript, targetSentence);
    };
      ...
      <div className="w-full max-w-md bg-[#F2EFE7] relative px-4">
        ...
        {activeSpeakingLesson && (
          <div className="fixed inset-0 z-50 flex flex-col bg-[#F2EFE7] w-full max-w-md mx-auto h-screen animate-fade-in p-4">
            <div className="flex justify-between items-center bg-white border-2 border-gray-200 rounded-2xl p-4 mb-6 shadow-sm mt-4">
              <button 
                onClick={() => { setActiveSpeakingLesson(null); setIsListening(false); }}
                className="text-[10px] font-black uppercase tracking-wider bg-gray-100 px-3.5 py-2 rounded-xl border border-gray-200 text-gray-500 hover:text-red-500 hover:bg-red-50 transition-colors"
              >
                ✕ Exit
              </button>
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full relative">
              ...
              <div className="bg-white border-2 border-[#10B981]/20 rounded-3xl p-6 shadow-lg text-center w-full max-w-[90%] relative">
                ...
                {/* 🔥 Button Ab English Answer Bolega */}
                <button 
              ...
              <div className="mt-10 mb-8">
                <button
                  onClick={() => startListening(activeSpeakingLesson.speakingData.english)}
                ...
                >
                  {isListening ? <Mic size={36} strokeWidth={2.5} /> : <MicOff size={36} strokeWidth={2.5} />}
                </button>
              </div>
