...
export default function EnglishAppUser() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4 font-sans pb-20 text-black">
      ...
      {/* 🎯 CONDITIONALLY RENDER PRACTICE OR PROFILE */}
      {isPracticeMode ? (
        /* PRACTICE MODE UI (Navbar remains visible now) */
        <div className="w-full max-w-md flex flex-col items-center mt-10 animate-in slide-in-from-bottom duration-500">
          ...
          <div className="w-full relative">
            ...
            >
                ...
                {showMeaning ? (
                <p className="mt-6 text-2xl font-black text-red-500 uppercase italic animate-in fade-in zoom-in duration-300 text-center">
                    {filteredWords[currentIndex]?.meaning}
                </p>
                ) : (
            ...
            <div className="absolute top-6 right-6">
              <PremiumSoundFeature isPremiumUser={isPremiumUser}>
                <button 
                    ...
                    className="w-12 h-12 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center active:scale-90 transition-all text-red-500"
                >
          ...
          {showMeaning ? (
            <div className="grid grid-cols-2 gap-3 mt-10 w-full px-2">
              <button onClick={() => handleReview(filteredWords[currentIndex]?._id, 'again')} className="p-4 bg-red-500 text-white rounded-2xl font-black uppercase text-[9px] shadow-lg shadow-red-100">Again (1m)</button>
              <button onClick={() => handleReview(filteredWords[currentIndex]?._id, 'hard')} className="p-4 bg-orange-500 text-white rounded-2xl font-black uppercase text-[9px] shadow-lg shadow-orange-100">Hard (6m)</button>
              <button onClick={() => handleReview(filteredWords[currentIndex]?._id, 'good')} className="p-4 bg-blue-500 text-white rounded-2xl font-black uppercase text-[9px] shadow-lg shadow-blue-100">Good (10m)</button>
              <button onClick={() => handleReview(filteredWords[currentIndex]?._id, 'easy')} className="p-4 bg-green-500 text-white rounded-2xl font-black uppercase text-[9px] shadow-lg shadow-green-100">Easy (3d)</button>
            </div>
          ) : (
