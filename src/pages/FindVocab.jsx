...
export default function PracticePage() {
  ...
  const generateNewChallenge = async () => {
    ...
    // Select random word from user's history
    ...
    try {
      ...
      if (response.ok && resData.success) {
        ...
        // Correct sentence ke words aur distractors ko mix kardo
        ...
        // Objects bana lo taaki unique ID rahe (same word do baar aaye to problem na ho)
        ...
        {currentChallenge && !isLoading && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="pt-6 border-t border-white/5 flex flex-col gap-4">
              {isCorrect !== null && (
                <div className={`p-4 rounded-xl text-center font-bold text-sm ${isCorrect ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
                  {isCorrect ? '🏆 Correct Translation! You nailed it.' : '❌ Oops! Sequence galat hai.'}
                </div>
              )}
