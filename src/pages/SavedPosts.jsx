...
export default function SavedPosts() {
  ...
  const [pronunciations, setPronunciations] = useState({}); // 🔥 Store Hindi text
  ...
  // 🔊 Audio Function (Sirf English Word)
  ...
  // 🔥 AI Sound Text Generator (English to Hindi Pronunciation)
  const fetchPronunciation = async (word) => {
    if (pronunciations[word]) return;
    try {
      const res = await fetch(`https://inputtools.google.com/request?text=${word}&itc=hi-t-i0-und&num=1`);
      const data = await res.json();
      if (data[0] === "SUCCESS") {
        let hindiText = data[1][0][1][0];
        // General Phonetic Fixes
        const refined = hindiText
          .replace(/इथेरेद/g, "विदरड")
          .replace(/थ/g, "द")
          .replace(/शिन/g, "शन")
          .replace(/तइ/g, "टाइ");
        setPronunciations(prev => ({ ...prev, [word]: refined }));
      }
    } catch (err) { console.error("Sound fetch failed"); }
  };
  ...
  const fetchSavedPosts = async () => {
    ...
    try {
      ...
      // Auto-fetch sound text for all saved posts
      posts.forEach(post => fetchPronunciation(post.word));
    } catch (err) { 
      console.error(err);
              ...
              <div className="w-full h-full flex items-center justify-center overflow-hidden">
                {item.type === 'video' ? (
                  ...
                  <div className="w-full h-full relative bg-black">
                    {playingIndex[post._id] === idx ? (
                      <iframe className="w-full h-full border-0" src={`${item.url.replace('watch?v=', 'embed/')}?autoplay=1`} allowFullScreen></iframe>
                    ) : (
                      <div className="absolute inset-0 flex items-center justify-center cursor-pointer" onClick={() => setPlayingIndex({...playingIndex, [post._id]: idx})}>
                         <div className="w-20 h-20 bg-red-600 rounded-full flex items-center justify-center text-white shadow-2xl">▶</div>
                      </div>
                    )}
                  </div>
                ) : (
                ...
                <div className="px-5 py-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-4">
                      <h2 className="text-5xl font-black text-gray-900 uppercase tracking-tighter italic leading-none">{post.word}</h2>
                      <button onClick={() => speakWord(post.word)} className="p-2 bg-gray-50 rounded-full active:scale-90 shadow-sm">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic font-bold">Sound:</span>
                      <span className="text-[14px] font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-xl border border-blue-100/50 uppercase tracking-tighter italic">
                        {pronunciations[post.word] || "🔊 AI Sound..."}
                      </span>
                    </div>

                    <p className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-red-600 to-orange-500 bg-clip-text text-transparent italic mt-1">{post.meaning}</p>
                  </div>
